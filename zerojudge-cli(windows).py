...
def submitCode():
    ...
    lang=raw_input('language(Default is CPP): ')
    if lang=='':
        lang='CPP'
    data['language']=lang
    filename=raw_input('Code file name without extension: ')
    codes=[]
    try:
...
while True:
    c=raw_input(cT.bcolors.OKBLUE+cT.bcolors.BOLD+'>> '+cT.bcolors.ENDC) 
    if c=='h':
        Help()
